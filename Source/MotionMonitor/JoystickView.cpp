...
void CJoystickView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Radio(pDX, IDC_RADIO_JOYSTICK, m_radioDevice);
}
...
BEGIN_MESSAGE_MAP(CJoystickView, CDockablePaneChildView)
	...
	ON_WM_DESTROY()
	ON_BN_CLICKED(IDC_RADIO_JOYSTICK, &CJoystickView::OnBnClickedRadioJoystick)
	ON_BN_CLICKED(IDC_RADIO_JOYPAD, &CJoystickView::OnBnClickedRadioJoypad)
END_MESSAGE_MAP()
...
bool CJoystickView::InitJoyStick()
{
	...
	RAWINPUTDEVICE rid;
	rid.usUsagePage = 1;
	rid.usUsage = (m_device==JOYSTICK)? 4: 5;	// Joystick
	rid.dwFlags = 0;
...
void CJoystickView::ParseRawInput(PRAWINPUT pRawInput)
{
	...
	for (i = 0; i < Caps.NumberInputValueCaps; i++)
	{
		...
		{
		case 0x30:	// X-axis (Roll)
			m_AxisX = (LONG)value - deviceLimit[m_device][0];
			break;

		case 0x31:	// Y-axis (Pitch)
			m_AxisY = (LONG)value - deviceLimit[m_device][0];
			break;

		case 0x32: // Z-axis (Yaw)
			m_AxisZ = (LONG)value - deviceLimit[m_device][0];
			break;
		
		// JoyPad
		case 0x33: // Rotate-Z (Yaw)
			m_AxisRz = (LONG)value - deviceLimit[m_device][1];
			break;

		// Joy Pad
		case 0x34: // Heave
			m_AxisH = (LONG)value - deviceLimit[m_device][1];
			break;

		case 0x35: // Rotate-Z (Yaw)
			m_AxisRz = (LONG)value - deviceLimit[m_device][1];
			break;
...
void CJoystickView::OnBnClickedRadioJoystick()
{
	UpdateData();
	m_device = JOYSTICK;
}
...
void CJoystickView::OnBnClickedRadioJoypad()
{
	UpdateData();
	m_device = JOYPAD;
}
