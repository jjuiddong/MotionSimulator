...
BEGIN_MESSAGE_MAP(CDirt3View, CDockablePaneChildView)
	...
	ON_BN_CLICKED(IDC_BUTTON_GAMEDATASAVE, &CDirt3View::OnBnClickedButtonGamedatasave)
END_MESSAGE_MAP()
...
void CDirt3View::OnBnClickedButtonGamedatasave()
{
	TCHAR name_filter[] = L"All Files (*.*)|*.*|GameData Files (*.csv)|*.csv|";
	CFileDialog ins_dlg(FALSE, L"csv", L"game.csv",
		OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR, name_filter, NULL);
	ins_dlg.m_ofn.nFilterIndex = 1;

	if (ins_dlg.DoModal() == IDOK)
	{
		cDirt3Controller::Get()->WriteGameData();

		CString dstFileName = ins_dlg.GetPathName();
		if (!CopyFile(L"game.csv", dstFileName, FALSE))
		{
			AfxMessageBox(L"Err Save GameData");
		}


		CString dstFTPFileName = _T("/www/gdi/");
		dstFTPFileName += ins_dlg.GetFileName();
		nsFTP::CFTPClient ftpClient;
		nsFTP::CLogonInfo logonInfo(L"jjuiddong.co.kr", 21, L"jjuiddong", L"ddong800");
		if (ftpClient.Login(logonInfo))
		{
			if (!ftpClient.UploadFile(L"game.csv", dstFTPFileName.GetBuffer()))
			{
				AfxMessageBox(L"Error!! FTP Upload");
			}
			else
			{
				AfxMessageBox(L"Success FTP Upload");
			}
		}
		else
		{
			AfxMessageBox(L"Error!! FTP Login");
		}
	}
}
